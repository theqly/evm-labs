...
int main() {
  ...
  //libusb_set_option(ctx, static_cast<libusb_option>(3)); //???
...
void printdev(libusb_device *dev) {
  ...
  if(handler) {
    r = libusb_get_string_descriptor_ascii(handler, desc.iSerialNumber, str, 256);
    if(r <= 0) {
      printf("nothing about iserialnumber \n");
    } else {
      printf("iserialnumber is %s\n", str);
    }

    r = libusb_get_string_descriptor_ascii(handler, desc.iProduct, str, 256);
    if(r <= 0) {
      printf("nothing about iserialnumber \n");
    } else {
      printf("iproduct number is %s\n", str);
    }

    r = libusb_get_string_descriptor_ascii(handler, desc.iManufacturer, str, 256);
    if(r <= 0) {
      printf("nothing about iserialnumber \n");
    } else {
      printf("imanufacturer is %s\n", str);
    }
  } else {
    printf("absolut nothing\n");
  }
